...
- 🎯 Entry points to execute your program using `python -m <flask_app>` or `$ flask_app` with basic CLI argument parsing.
...
Awesome flask_app created by darebucad
...
```bash
git clone https://github.com/darebucad/flask-app flask_app
cd flask_app
make install
```
...
```bash
flask_app create-db   # run once
flask_app populate-db  # run once (optional)
flask_app add-user -u admin -p 1234  # ads a user
flask_app run
```
