...
original_author="darebucad"
original_name="flask_app"
original_urlname="flask-app"
original_description="Awesome flask_app created by darebucad" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_app "${name}"
...
then
    ...
    echo "then run 'flask_app' or 'python -m flask_app'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_app' or 'python -m flask_app' or 'poetry run flask_app'"
fi
